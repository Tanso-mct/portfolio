...
(
    ...
    for (const std::unique_ptr<wb::IOptionalValue> &id : (args.entityIDView_)(wbp_render::MeshRendererComponentID()))
    {
        ...
        wbp_d3d12::UpdateBuffer
        (
            meshRenderer->GetColorConfigBuffer(),
            &meshRenderer->GetColorConfig(), sizeof(wbp_render::ModelColorConfig)
        );
        commandList->SetGraphicsRootConstantBufferView
        (
            ROOT_SIGNATURE_COLOR_CONFIG_BUFFER,
            meshRenderer->GetColorConfigBuffer()->GetGPUVirtualAddress()
        );
        ...
        size_t diffuseTextureAssetID = 0;
        if (meshRenderer->GetTextureAssetID(wbp_render::TextureType::Diffuse, diffuseTextureAssetID))
        {
            // Already asset container's mutex is locked by ThreadSafeGet, so we can safely access the asset
            wb::IAsset *textureAsset = assetContainer.PtrGet(diffuseTextureAssetID);
            if (textureAsset == nullptr)
            {
                std::string err = wb::CreateErrorMessage
                (
                    __FILE__, __LINE__, __FUNCTION__,
                    { "Texture asset is not loaded for ID: ", std::to_string(diffuseTextureAssetID) }
                );
                wb::ConsoleLogErr(err);
                wb::ErrorNotify("WBP_RENDER", err);
                wb::ThrowRuntimeError(err);
            ...
            if (texture == nullptr)
            {
                ...
                (
                    __FILE__, __LINE__, __FUNCTION__,
                    { "The asset is not a TextureAsset for ID: ", std::to_string(diffuseTextureAssetID) }
                );
                wb::ConsoleLogErr(err);
                wb::ErrorNotify("WBP_RENDER", err);
                wb::ThrowRuntimeError(err);
            ...
            size_t diffuseTextureSRVSlot = diffuseTextureAssetID + 1;

            if (textureSRVMap_.find(diffuseTextureSRVSlot) == textureSRVMap_.end())
            {
                ...
                textureSRVMap_[diffuseTextureSRVSlot] = currentSRVSlotSize_;
            ...
            (
                ...
                (
                    ...
                    textureSRVMap_[diffuseTextureSRVSlot], 
        ...
        {
            // Set dummy texture if no diffuse texture is set
            commandList->SetGraphicsRootDescriptorTable
            (
                ROOT_SIGNATURE_TEXTURE_SRV,
                CD3DX12_GPU_DESCRIPTOR_HANDLE
