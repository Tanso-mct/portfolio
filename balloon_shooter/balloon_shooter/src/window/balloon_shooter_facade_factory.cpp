...
std::unique_ptr<wb::IWindowFacade> balloon_shooter::BalloonShooterWindowFacadeFactory::Create() const
{
...

    {
        ...
        context->Name() = L"Balloon Shooter";
        context->Width() = 960;
        context->Height() = 540;
        context->Style() &= ~WS_THICKFRAME;
        context->Style() &= ~WS_MINIMIZEBOX;
    ...
    facade->AddMonitorID(balloon_shooter::BalloonShooterKeyboardMonitorID());
    facade->AddMonitorID(balloon_shooter::BalloonShooterMouseMonitorID());
