...
public:
    ...
    // Get the material by handle
    virtual const render_graph::Material* GetMaterial(const render_graph::MaterialHandle* material_handle) const = 0;
};
...
protected:
    ...
    const render_graph::Material* GetMaterial(const render_graph::MaterialHandle* material_handle) const override;
...
private:
    ...
    std::unique_ptr<render_graph::MaterialHandleManager> material_handle_manager_ = nullptr;
    std::unique_ptr<render_graph::MaterialHandleKeyGenerator> material_handle_key_generator_ = nullptr;
