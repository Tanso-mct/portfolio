...
#include "mono_forge_app_template/include/components.h"
...
#include "mono_forge_app_template/include/service_hub.h"
...
std::unique_ptr<mono_forge_app_template::ECSHub> InitializeECS(mono_service::ServiceProxyManager& service_proxy_manager);
...
#endif
{
    ...
    if (FAILED(hr))
    {
        ...
        return mono_forge_app_template::MAIN_ERROR; // COM initialization failed
    }

    // Initialize service
    std::unique_ptr<mono_forge_app_template::ServiceHub> service_hub = mono_forge_app_template::InitializeService();
    if (!service_hub)
    {
        utility_header::ConsoleLogErr({ "Service initialization failed!" }, __FILE__, __LINE__, __FUNCTION__);
        return mono_forge_app_template::MAIN_ERROR; // Service initialization failed
    }

    // Initialize ECS
    std::unique_ptr<mono_forge_app_template::ECSHub> ecs_hub = mono_forge_app_template::InitializeECS(service_hub->GetServiceProxyManager());
    if (!ecs_hub)
    {
        utility_header::ConsoleLogErr({ "ECS initialization failed!" }, __FILE__, __LINE__, __FUNCTION__);
        return mono_forge_app_template::MAIN_ERROR; // ECS initialization failed
    }

    // Create initial entities and components
    if (!mono_forge_app_template::CreateInitialEntities(ecs_hub->GetWorld(), service_hub->GetServiceProxyManager()))
    {
        utility_header::ConsoleLogErr({ "Initial entity creation failed!" }, __FILE__, __LINE__, __FUNCTION__);
        return mono_forge_app_template::MAIN_ERROR; // Initial entity creation failed
    }

    // Run message loop
    bool is_runnning = true;
    mono_forge_app_template::RunMessageLoop(is_runnning, [&]() -> bool
    {
        // Update systems
        if (!ecs_hub->Update(mono_forge_app_template::g_system_execution_order))
            return false;
    ...
    return mono_forge_app_template::MAIN_SUCCESS;
}
    ...
    // Import asset service
    {
        ...
        if (!service_hub->Import<
            mono_asset_service::AssetService, mono_asset_service::AssetServiceHandle>(
                mono_forge_app_template::AssetThreadAffinityHandle::ID(), param))
            return nullptr;
    }
    ...
    // Import transform service
    {
        ...
        if (!service_hub->Import<
            mono_transform_service::TransformService, mono_transform_service::TransformServiceHandle>(
                mono_forge_app_template::MainThreadAffinityHandle::ID(), param))
            return nullptr;
    }
...
{
    std::unique_ptr<mono_forge_app_template::ECSHub> ecs_hub = std::make_unique<mono_forge_app_template::ECSHub>();
