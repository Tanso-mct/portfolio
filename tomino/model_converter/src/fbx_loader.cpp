...
std::unique_ptr<IFileData> FBXLoader::Load(std::string_view file_path) const
{
    ...
    {
        fbxsdk::FbxNode* child_node = root_node->GetChild(child_index);
        if (!child_node) continue;

        // メッシュノードかどうかをチェック
        fbxsdk::FbxNodeAttribute* node_attr = child_node->GetNodeAttribute();
        if (!node_attr) continue;
        if (node_attr->GetAttributeType() != fbxsdk::FbxNodeAttribute::eMesh) continue;

        // メッシュノードからメッシュを取得
        fbxsdk::FbxMesh* mesh = child_node->GetMesh();
        if (!mesh) continue;

        // マテリアル要素を取得
        const FbxGeometryElementMaterial* material_element = mesh->GetElementMaterial();
        ...
        if (!element_uv) continue; // UVが存在しない場合はスキップ

        const fbxsdk::FbxGeometryElementNormal* element_normal = mesh->GetElementNormal(0);
        if (!element_normal) continue; // 法線が存在しない場合はスキップ

        const fbxsdk::FbxGeometryElementTangent* element_tangent = mesh->GetElementTangent(element_tangent_index);
        if (!element_tangent) continue; // 接線が存在しない場合はスキップ

        // グローバル変換行列を取得
        fbxsdk::FbxAMatrix global_matrix = child_node->EvaluateGlobalTransform();
        ...
        {
            // マテリアルインデックスを取得
            int material_index = material_element->GetIndexArray().GetAt(polygon_index);

            // マテリアル名を取得
            fbxsdk::FbxSurfaceMaterial* material = child_node->GetMaterial(material_index);
            ...
            for (int vertex_index = 0; vertex_index < polygon_size; ++vertex_index)
            {
                ...
                vertex.position_ = DirectX::XMFLOAT3(static_cast<float>(pos[0]), static_cast<float>(pos[1]), static_cast<float>(pos[2]));
                ...
                case fbxsdk::FbxGeometryElement::eByControlPoint:
					...
					case fbxsdk::FbxGeometryElement::eDirect:
                    {
                        ...
                        vertex.uv_ = DirectX::XMFLOAT2(static_cast<float>(uv[0]), static_cast<float>(uv[1]));
                    }
						break;
                ...
                // 法線を取得
				{
                    ...
                    // 正規化してfloat3へ
                    DirectX::XMFLOAT3 n(static_cast<float>(transformed_normal[0]), static_cast<float>(transformed_normal[1]), static_cast<float>(transformed_normal[2]));
                ...
                if(element_tangent->GetMappingMode() == fbxsdk::FbxGeometryElement::eByPolygonVertex)
				{
					...
					case fbxsdk::FbxGeometryElement::eDirect:
                    {
                        ...
                        vertex.tangent_ = {static_cast<float>(tangent[0]), static_cast<float>(tangent[1]), static_cast<float>(tangent[2])};
                    }
						break;
